...
function fetchList(name, status = 1) {
  return new Promise((resolve, reject) => {
    request(
      `https://myanimelist.net/animelist/${name}?status=${status}`,
      (err, resp, body) => {
...
function fetchBecause() {
  return new Promise((resolve, reject) => {
    if (becauseCache) return resolve(becauseCache);

    request(
      'https://bcmoe.blob.core.windows.net/assets/us.json',
      (err, resp, body) => {
        if (err) return reject(err);
        try {
...
function searchCrunchyroll(title) {
  return new Promise((resolve, reject) => {
    request(
      `http://www.crunchyroll.com/search?from=search&q=${title.replace(
        / /g,
        '+'
      )}`,
      (err, resp, body) => {
...
function searchMAL(term) {
  return new Promise((resolve, reject) => {
    request(
      `https://myanimelist.net/search/all?q=${term}`,
      (err, resp, body) => {
        if (err) {
          return reject(err);
        }
        ...
        const $ = cheerio.load(body);
        const anime = $(
          '.content-left h2#anime + article .list .information a.fw-b'
        );

        resolve(
          Array.from(anime).map(e => [e.attribs.href, e.children[0].data])[0]
        );
      }
    );
  });
...
function guessFromMAL(mal_item) {
  return new Promise((resolve, reject) => {
    ...
    if (becauseRes) return resolve(becauseRes);

    // Open the MAL show page and find the "English" or "Synonyms" section on the side bar
    request(
      `https://myanimelist.net${encodeURI(anime_url)}`,
      async (err, resp, body) => {
        if (err) {
...
async function todo(options) {
  ...
  const malPromise = fetchList(
    config.settings.myanimelist.username,
    options.ptw ? 6 : 1
  );
  ...
  return (await malPromise).map(show => {
    ...
    if (options.images) base.image = show.anime_image_path;

    if (show.anime_airing_status === 1) {
      const meta = _.find(airing, { meta: { mal: show.anime_id } }) || {
        next: 0,
        meta: { id: -1 },
      };
      return {
...
async function airing() {
  const malPromise =
    config &&
    Promise.all([
      fetchList(config.settings.myanimelist.username, 1),
      fetchList(config.settings.myanimelist.username, 6),
    ]);
  const airingCalendar = await anilist.calendar();
  const mal = (malPromise ? [].concat(...(await malPromise)) : []).filter(
    s => s.status !== 4 && s.status !== 2
  );
  const mal_obj = mal.reduce((obj, a) => ({ ...obj, [a.anime_id]: a }), {});

  for (const shows of Object.values(airingCalendar)) {
    for (const show of shows) {
      ...
      show.onMyConfig =
        ...
        config.shows &&
        crLink &&
        _.find(config.shows, s => s.crunchyroll.match(crLink.url));
    }
  }
