...
const TEMP_BATCH_PATH =
  (config ? fs.realpathSync(config.settings.output_dir) + '/' : '') +
  '.crunchybatch.txt';
...
function runCrunchy(...args) {
  crunchy(
    (process.argv = [
      ...
      '--output',
      fs.realpathSync(config.settings.output_dir),
      '--ignoredub',
    ...
    err => {
      ...
      fs.existsSync(TEMP_BATCH_PATH) &&
        fs.unlink(TEMP_BATCH_PATH, err => {
...
async function watchFeed() {
  ...
  const toDownload = items
    .filter(i => i['crunchyroll:episodeNumber'])
    .map(i => ({
      date: i.pubDate[0],
      episode: +i['crunchyroll:episodeNumber'][0],
      link: i.link,
      ...(_.find(config.shows || [], {
        crunchyroll: i.link[0].match(CR_URL_REGEX)[0],
      }) || {}),
    }))
    .filter(i => i.title)
    .filter(i => {
